...
static struct dirtable _CDVD_OpenDir;
static struct dirtable *_CDVD_pCurrentOpenDir;
...
int CDVD_dopen( int kernel_fd, char *path);
int CDVD_dclose( int kernel_fd);
int CDVD_dread( int kernel_fd, char *buf);
...
int T_CDVD_dopen(int kernel_fd, char *path)
{
	register int ret;
	StoreGP();
	ret=CDVD_dopen(kernel_fd,path);
	RestoreGP();
	return ret;
}

int T_CDVD_dclose(int kernel_fd)
{
	register int ret;
	StoreGP();
	ret=CDVD_dclose(kernel_fd);
	RestoreGP();
	return ret;
}


int T_CDVD_dread(int kernel_fd, char *buf)
{
	register int ret;
	StoreGP();
	ret=CDVD_dread(kernel_fd, buf);
	RestoreGP();
	return ret;
}
...
void CDVD_init( struct fileio_driver *driver)
{
	printf("CDVD: CDVD Filesystem v1.14\n");
...
int CDVD_dopen( int kernel_fd, char *path)
{
	struct dirtable *pDir;

	if (_CDVD_pCurrentOpenDir != NULL)
	{
		// we only support one open dir at a time now unfortunately
		return -1;
	}

	#ifdef DEBUG
	printf("cdvd: dopen %s (%d)\n", path, sizeof(struct fio_dirent));
	#endif

	// pre-cache the dir (and get the new pathname - in-case selected "..")
	if (CDVD_Cache_Dir(path, CACHE_START) != TRUE)
	{
		#ifdef DEBUG
			printf("CDVD_dopen - Call of CDVD_Cache_Dir failed\n");
		#endif
		return -1;
	}

	// open dir
	_CDVD_pCurrentOpenDir = &_CDVD_OpenDir;

	pDir = _CDVD_pCurrentOpenDir;

	(char*)pDir->tocEntryPointer = CachedDirInfo.cache;
	pDir->dir_entry = 0;

/*
	// skip the first self-referencing entry
	(char*)pDir->tocEntryPointer += pDir->tocEntryPointer->length;
...
int CDVD_dclose( int kernel_fd)
{
	struct dirtable *pDir = _CDVD_pCurrentOpenDir;

	#ifdef DEBUG
  	printf("cdvd: dclose %d\n", kernel_fd);
	#endif
	...
	pDir->tocEntryPointer = NULL;

	// close open dir
	_CDVD_pCurrentOpenDir = NULL;

	return 1;
}

void DirEntryCopy(fio_dirent_t *dirent, struct TocEntry *toc_entry)
{
	dirent->stat.addr = toc_entry->fileLBA;
	dirent->stat.attr = (toc_entry->fileProperties & 0x02) ? FIO_ATTR_SUBDIR : 0;
	dirent->stat.attr |= FIO_ATTR_READABLE;
	dirent->stat.size =  toc_entry->fileSize;
	dirent->stat.hisize = 0;

	// copy file name
  	memcpy((char *)dirent->name, (const char *)toc_entry->filename, sizeof(dirent->name));
...
int CDVD_dread( int kernel_fd, char *buf)
{
	...
	struct dirtable *pDir = _CDVD_pCurrentOpenDir;
	fio_dirent_t *dirent = (fio_dirent_t *)buf;

	// clear output buffer
	memset((char *)dirent, 0, sizeof(dirent));


	if (pDir == NULL || pDir->tocEntryPointer == NULL)
	{
		// no dir open
		return -1;
	}

	if (pDir->tocEntryPointer->length == 0)
	{
		...
		(char*)pDir->tocEntryPointer = CachedDirInfo.cache + (((((char*)pDir->tocEntryPointer - CachedDirInfo.cache)/2048)+1)*2048);
	}

	if ((char*)pDir->tocEntryPointer >= CachedDirInfo.cache + (CachedDirInfo.cache_size * 2048))
	{
		...
		if ((CachedDirInfo.cache_offset+CachedDirInfo.cache_size) < CachedDirInfo.sector_num)
		{
			...
			{
				// failed to cache next block 
				pDir->tocEntryPointer = NULL;
				return -1;
			}
		} else
		{
			// no more to dir entries
			pDir->tocEntryPointer = NULL;
			return -1;
		}
	...
	// copy dir entry
	TocEntryCopy(&localTocEntry, pDir->tocEntryPointer);

	switch (pDir->dir_entry)
	{
		case 0:
	...
	DirEntryCopy(dirent, &localTocEntry);

	// advance to next entry
	(char*)pDir->tocEntryPointer += pDir->tocEntryPointer->length;
	pDir->dir_entry++;

//	printf("cdvd: dread %d %s\n", kernel_fd, localTocEntry.filename);
...
int _start( int argc, char **argv)
{
	...
	filedriver_functarray[ FIO_DOPEN ] = T_CDVD_dopen;
	filedriver_functarray[ FIO_DREAD ] = T_CDVD_dread;
	filedriver_functarray[ FIO_DCLOSE ] = T_CDVD_dclose;
...
int CDVD_Cache_Dir(const char* pathname, enum Cache_getMode getMode)
{
	...
	#endif
//	printf("CDVD_Cache_Dir %s %d\n", pathname, sizeof(fio_dirent_t));
...
void TocEntryCopy(struct TocEntry* tocEntry, struct dirTocEntry* internalTocEntry)
{
	...
	{
		// strip the ;1 from the filename (if it exists)
		if (tocEntry->filename[filenamelen - 2] == ';')
		{
			tocEntry->filename[filenamelen - 2] = 0;
		}
	}
